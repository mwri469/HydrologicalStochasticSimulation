...
OUTPUT_DIR = Path(r"P:\1011941\1011941.3000\WorkingMaterial\01 IPCC6 MODELLING\02 DELTA METHOD\02 OUTPUT\MAWR Synthetic method")
...
def disaggregate_19month_to_monthly(rolling_19m_series: pd.DataFrame, 
    ...
    for idx, row in rolling_19m_series.iterrows():
        ...
        vcsn_sequence_start = vcsn_with_rolling.loc[selected_idx, "YearMonth"]
        ...
        target_indices = monthly_extended[
            ...
            (monthly_extended["YearMonth"] < target_ym + pd.DateOffset(months=ROLLING_WINDOW))
        ].index.tolist()
